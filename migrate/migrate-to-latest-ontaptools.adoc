...
NOTE: ONTAP tools for VMware vSphere 9.xxのセットアップは、VASA Provider機能のみで実行する場合にのみ移行することを推奨します。


NOTE: ONTAP tools for VMware vSpher 9.xから10.3に移行したあと、ONTAP tools 10.3ではVMFSデータストアでNVMe-oFのみがサポートされるため、NVMe/FCプロトコルを使用するvVolデータストアが機能しません。

* このタスクについて *

ONTAP tools for VMware vSphere 9.12D1および9.13D2リリースから10.3リリースへの移行がサポートされます。
...
. ONTAP tools for VMware vSphere 10.3リリースに移行するvCenter Serverインスタンスを追加します。を参照し link:../configure/add-vcenter.html["vCenter Serverインスタンスの追加"]
. ONTAP Tools for VMware vSphereプラグインvCenter Server APIを使用して、ストレージバックエンドをローカルにオンボードします。移行用に、ローカルスコープのストレージとしてストレージを追加します。
. ONTAP tools for VMware vSphere 9.xxから移行されたNFSデータストアとVMFSデータストアがONTAP tools for VMware vSphere 10.3に表示されるのは、データストアの検出ジョブがトリガーされたあとだけです。トリガーされるまでに最大30分かかることがあります。ONTAP tools for VMware vSphere Plugin UIページの[Overview]ページにデータストアが表示されるかどうかを確認します。
...
.. 省略記号メニューをクリックし、*設定のリセット*をクリックします。
.. 省略記号メニューをクリックし、*削除*を選択します。
+
...
----
. ONTAP tools for VMware vSphere 10.3リリース向けOVAを導入します。
. ONTAP tools for VMware vSphere 10.3リリースに移行するvCenter Serverインスタンスを追加します。を参照して link:../configure/add-vcenter.html["vCenter Serverインスタンスを追加する"]
...
+
古いVASA Providerが停止すると、vCenter ServerはONTAP tools for VMware vSphereにフェイルオーバーします。すべてのデータストアとVMにアクセスし、ONTAP Tools for VMware vSphereからアクセスできます。
...
以前のリリースのONTAP tools for VMware vSphereから10.3リリースに移行したら、SRAアダプタを再スキャンして、[VMware Live Site Recovery][Storage Replication Adapters]ページで詳細が更新されたことを確認します。
