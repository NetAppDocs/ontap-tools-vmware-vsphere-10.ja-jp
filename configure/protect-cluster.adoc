...
+
AutomatedFailOverDuplexポリシータイプには、均一なホスト構成と非均一なホスト構成があります。*Uniform host configuration*トグルボタンを選択すると、ホストイニシエータグループ構成がターゲットサイトに暗黙的に複製されます。詳細については、link:../concepts/ontap-tools-concepts-terms.html["主要な概念と用語"]を参照してください。
