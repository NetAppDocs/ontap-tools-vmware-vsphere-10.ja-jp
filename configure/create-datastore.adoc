...
. アドバンストオプションで、NFSプロトコルには*[カスタムエクスポートポリシー]*を、iSCSIプロトコルとFCプロトコルにはカスタムのイニシエータグループ名を選択できますが、この設定は推奨されません。このオプションを使用する場合は、すべてのオブジェクトに対してvCenterで検出を実行してください。
+
...
. アドバンストオプションでカスタムのエクスポートポリシーを選択することもできますが、この方法は推奨されません。使用する場合は、すべてのオブジェクトに対してvCenterで検出を実行してください。
+
