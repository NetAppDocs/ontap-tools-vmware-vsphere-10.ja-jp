...
+
アプライアンスが 1 つの vCenter インスタンスでホストされ、別のインスタンスを管理する 2 つの vCenter Server トポロジでONTAPツールを展開する場合は、 ONTAPツールをホストする vCenter インスタンスに制限付きロールを割り当てることができます。  OVF テンプレートの展開に必要な、以下にリストされているデータストア権限のみを持つ専用の vCenter ユーザーとロールを作成できます。
