...
ONTAP tools for VMware vSphere 10.4以降、ONTAP toolsはONTAPおよびvCenterオブジェクトを自動的に作成および維持し、VMwareデータセンター環境におけるデータストア管理を簡素化します。
...
NOTE: 再利用できるのはカスタムigroupのみです。ONTAP tools 9.xからの移行中に作成されたigroup（移行されたigroup）は再利用できず、ONTAP tools 10.5P2以降では、使用されなくなると自動的に削除されます。
