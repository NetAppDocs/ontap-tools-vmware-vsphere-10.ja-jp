...
* 導入時に、CPUホットアドとメモリホットプラグが有効になっていることを確認します。導入時に有効になっていなかった場合は、vCenter Clientのメニューで有効にすることができます。vCenterクライアントから有効にするには、次の手順に従います。
+
** [インベントリ]*の下にあるONTAP tools for VMware vSphere仮想マシンに移動し、*[設定の編集]*を選択します。
