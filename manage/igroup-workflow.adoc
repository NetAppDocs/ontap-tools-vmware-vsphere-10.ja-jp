...
ONTAP tools 10.5P2以降では、移行されたigroupは使用されなくなると削除されます。以前のバージョンでは、移行されたigroupは削除されませんでした。以下の点に注意してください：

* 移行されたigroupは、データストア間で再利用できません。
* カスタムigroup（ユーザー定義）は削除されず、引き続き再利用できます。
